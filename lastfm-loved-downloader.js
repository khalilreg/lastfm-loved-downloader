...
for (let index = 0; index < lastFmLovedSongs.length; index++) {
  ...
  await ytmusic.searchSongs(`${lovedSong.artist} ${lovedSong.name}`).then(async songs => {
    ...
    execSync(`yt-dlp --embed-thumbnail --embed-metadata -f bestaudio --extract-audio --audio-quality 0 --audio-format opus -o \"${filePath}\" https://music.youtube.com/watch?v=${songs[0].videoId}`, (error, stdout, stderr) => {
      if (error) {
